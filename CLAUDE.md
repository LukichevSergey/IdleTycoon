...
- **`js/core/cloudSync.js`** — cross-device saves: an e-mail account and one Firestore document per player. Talks to Firebase over **REST, deliberately not via the JS SDK** — the SDK is ~250 KB and its auth requires an authorized domain, which a page opened as `file://` does not have; the REST endpoints are CORS-open, so the cloud works in both launch modes and the project keeps its zero-dependency rule. `FirestoreSaveProvider` is a `StorageProvider`, so wrapping it in a `StorageManager` gives cloud saves the same versioning and migrations as local ones. The whole save goes into **one base64 string field** — Firestore forbids nested arrays, and the quote history is exactly that. `js/cloudConfig.js` holds `apiKey` + `projectId`; empty values simply disable the feature. That `apiKey` is **not a secret** (Firestore rules do the access control) and is meant to be committed.

  Conflicts are resolved by `pushId`, not by timestamps. The cloud doc carries the id of the last upload and each device remembers the id it last synced; `remoteIsNew` is true only when someone *else* pushed. Comparing `lastTimestamp` instead would pop the conflict dialog on every single-device launch, since the local save is always a few minutes fresher than the last cloud push.
...
- **`js/main.js`** — composition root: wires state↔storage↔UI, runs the loops (game tick 1s with real-elapsed `dt` so background-tab throttling doesn't lose income; market tick 3s; autosave 30s; cloud upload 3 min and on `visibilitychange` → hidden, since mobile browsers often skip `beforeunload`; random events every 15–40 min).

  Startup order is load-bearing: `_resolveStartupSave()` picks between the local and cloud save **first**, and only then `_applyOfflineFrom()` accrues offline income from the winner's `lastTimestamp`. Computing offline before the cloud is consulted shows the player a payout that a newer cloud save then wipes out a second later.

  Offline income is granted for **cloud** loads (startup and the manual button) but not for file import — a save is state frozen at its `lastTimestamp`, so counting it forward is the same physics as reopening the game on another device, and it duplicates nothing since loading replaces the whole state. File import stays exempt so an old file can't be treated as an idle bonus.
