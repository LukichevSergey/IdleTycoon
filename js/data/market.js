 ...
 *   Облигации ограничены объёмом выпуска.
 *
 * Крипта в этой иерархии НЕ участвует — это казино, а не инструмент
 * дохода. У неё нет ни купона, ни дивидендов, а growth сделан
 * ОТРИЦАТЕЛЬНЫМ: якорь медленно сдувается, поэтому «купил и забыл»
 * статистически проигрывает (медиана суточного холда −1…−10%, в плюсе
 * меньше половины прогонов — примерно как красное/чёрное в рулетке).
 * Весь смысл крипты — огромный разброс: за сутки монета ходит от −85%
 * до +400% и выше. Зарабатывают на ней таймингом (покупка при mood<1),
 * а не удержанием. Не делайте growth крипты положительным: тогда
 * достаточно долгий холд выигрывает гарантированно и крипта
 * превращается в медленную ренту.
 * Характеры монет заданы шириной коридора и vol: ЦФК — «солидная»,
 * ЭФР — поживее, ДОГ — безумный мем-койн (может упасть в 20 раз).
 *
...
// growth здесь отрицательный намеренно (см. комментарий в шапке файла):
// холд не должен приносить доход, весь заработок — на колебаниях mood.
const CRYPTO_DEFS = [
  { id: "cfk", kind: "crypto", ticker: "ЦФК", name: "Цифрокойн", sector: "Криптовалюта", basePrice: 52000, vol: 0.022, growth: -0.03, div: 0, moodRange: [0.25, 4],  step: 0.0001 },
  { id: "efr", kind: "crypto", ticker: "ЭФР", name: "Эфирион",   sector: "Криптовалюта", basePrice: 3900,  vol: 0.032, growth: -0.05, div: 0, moodRange: [0.15, 7],  step: 0.001 },
  { id: "dog", kind: "crypto", ticker: "ДОГ", name: "ДогМонета", sector: "Мем-койн",     basePrice: 2.4,   vol: 0.05,  growth: -0.09, div: 0, moodRange: [0.05, 20], step: 1 },
];
